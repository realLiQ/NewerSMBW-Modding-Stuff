...
bool daSidestepper_c::collisionCat9_RollingObject(ActivePhysics *apThis, ActivePhysics *apOther) {
	...
	if (acState.getCurrentState() == &StateID_Bury || acState.getCurrentState() == &StateID_BackUp || acState.getCurrentState() == &StateID_Run)
	{
		destroyBarrel(blah);
		return true;
	}
...
void daSidestepper_c::beginState_Projectiles()
{
	...
	this->rndmtimerock = GenerateRandomNumber(4);
...
void daSidestepper_c::executeState_Projectiles()
{
	...
	if (this->right == true)
	{
		...
		if (round0)
		{
			...
			if (timer == 217)
			{
				Vec interneffectpos = { pos.x + 12.0, pos.y + 7.0, 0 };
...
void daSidestepper_c::beginState_Actors()
{
	...
	//Code with help from Ninji and Mihi:
	if ((dStageActor_c*)fBase_c::search(418) != NULL) {
		destroyBarrel(barrel);
	}
	else {
		PlaySound(this, 179);
...
void daSidestepper_c::executeState_Actors()
{
	...
	if (timer > 50)
	{
		if (timer < 74)
		{
			if (timer == 51)
			{
				this->barrel = dStageActor_c::create(418, 0, this->posbarrel, 0, 0);

				if (target->pos.x > pos.x)
				{
					this->enemy = CreateActor(286, 0, this->posenemy, 0, 0);
				}
				else
				{
					this->enemy = CreateActor(286, 1 << 0, this->posenemy, 0, 0);
				}
			}
			barrel->pos.y += 2.0;
			barrel->pos.y += 2.0;
